...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
 //====================================== materials ==============================================
  ...
  G4Material* Vacuum = material->FindOrBuildMaterial("G4_Galactic"); 
  ...
  G4Box* world = new G4Box("worldSolid",worldX,worldY,worldZ);
  ...
  G4double translateX  = 16.93*mm;     // translate all detectors on X axis for the beam to hit between first 2 sectors of the sensors
  G4double translateY  = -137.6*mm;    // translate all detectors on Y axis for the beam to hit in the middle of the sensors
//  G4double translateY  = -126.8*mm;    // translate all detectors on Y axis for the beam to hit in 26 pad (counting from the bottom pad and on the bottom there are the shortest pads)

//  G4double tranTY = 0*mm;              // tranzitie pentru placile de w
  G4double tranTY = 10.8*mm;
  ...
  G4double dAbsAlfoil = (wSlot + dSlotfAl)*mm;              // = 62.5 mm
     AlfoilPhys = new G4PVPlacement(0, G4ThreeVector(translateX,0,-(dAbsAlfoil + wfoilAl/2)*mm), AlfoilLog, "AluminiumFoil", worldLog, false, 0, fCheckOverlaps);
  ...
  for( int b = 0; b < 5; b++ ){    
     teleplanLog[b] = new G4LogicalVolume(telescope, si, "teleplanLog[b]");
       teleplanPhys = new G4PVPlacement(0, G4ThreeVector(0, 0, ZTelPlans[b]*mm), teleplanLog[b], "TelescopePlan", worldLog, false, b, fCheckOverlaps);
  }  
  ...
  G4double Zscint1 = - (-ZTelPlan1 + telescopeZ/2 + dScint2Tel1 + dScint2Scint1 + scintillatorZ + scintillatorZ/2)*mm;
    ...
    scintplanPhys1 = new G4PVPlacement(0, G4ThreeVector(0.*mm, 0, Zscint1*mm), scintplanLog, "ScintillatorPlan1", worldLog, false, 1, fCheckOverlaps);
    scintplanPhys2 = new G4PVPlacement(0, G4ThreeVector(0.*mm, 0, Zscint2*mm), scintplanLog, "ScintillatorPlan2", worldLog, false, 2, fCheckOverlaps);
  ...
  G4Box* magnet = new G4Box("magnet",mX,mY,mZ/2);
     ...
     magnetPhys = new G4PVPlacement(0,G4ThreeVector(0,0,wm*mm),magnetLog,"Magnet",worldLog,false,0,fCheckOverlaps);
    */        
  ...
  G4double ZAbsorber[15] = { 3.470*mm, 3.542*mm, 3.505*mm, 3.490*mm, 3.584*mm, 3.521*mm, 3.645*mm, 3.470*mm, 3.550*mm, 3.558*mm, 3.543*mm, 3.543*mm, 3.550*mm, 3.550*mm, 3.528*mm };      // each absorber plate has a different thickness
  ...
  for( int k = 0; k < 15; k++ ){    
     ...
     absorberPhys[k] = new G4PVPlacement(0, G4ThreeVector(translateX, tranTY, wW0), absorberLog[k], "TungstenAbsorber", worldLog, false, k, fCheckOverlaps);
  }    
  ...
  //silicon layer - to identify pads we use replica of SensorPad and SenzorSector inside Sensor
  G4Tubs *solidSensor = new G4Tubs("solidSensor", KRint, KRext, wSi/2, KstartAngle, KtotAngle);
      logicCalSensor = new G4LogicalVolume(solidSensor, si, "logicCalSensor");
  G4Tubs *solidSensorSector = new G4Tubs("solidSensorSector", KRint, KRext, wSi/2, -3.75*deg, 7.5*deg);
      logicCalSector = new G4LogicalVolume(solidSensorSector, si, "logicCalSector");
  ...
  new G4PVReplica("SiCalSector", logicCalSector, logicCalSensor, kPhi, 4, 7.5*deg, KstartAngle);
  new G4PVReplica("SiCalPad", fLogicCalPad, logicCalSector, kRho, 64, 1.8*mm, KRint);
  ...
  //--------------------------------- construct full sensor --------------------------------
  for( int i = 0; i < 16; i++ ) {
      G4Box* fullsensorBox = new G4Box("fullsensorBox", absorberX, absorberY, wSiSensorAll/2);
      fullsensorLog[i]  = new G4LogicalVolume(fullsensorBox, Vacuum, "fullsensorLog[i]");
      fullsensorPhys[i] = new G4PVPlacement(0, G4ThreeVector(translateX, tranTY, wSlot*i + wSiSensorAll/2), fullsensorLog[i], "FullSensor", worldLog, false, i, fCheckOverlaps);
      
      
      stratK1_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZK1*mm), stratK1_log, "K1", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAd1_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAd1*mm), stratAd1_log, "Ad1", fullsensorLog[i], false, i, fCheckOverlaps);
      stratCu1_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZCu1*mm), stratCu1_log, "Cu1", fullsensorLog[i], false, i, fCheckOverlaps);
      stratK2_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZK2*mm), stratK2_log, "K2", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAd2_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAd2*mm), stratAd2_log, "Ad2", fullsensorLog[i], false, i, fCheckOverlaps);
      stratK3_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZK3*mm), stratK3_log, "K3", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAraldite1_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAralditel1*mm), stratAraldite1_log, "Araldite1", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAl1_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAl1*mm), stratAl1_log, "Al1", fullsensorLog[i], false, i, fCheckOverlaps);
      stratSi_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZSi*mm), logicCalSensor, "Si", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAl2_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAl2*mm), stratAl2_log, "Al2", fullsensorLog[i], false, i, fCheckOverlaps);
      stratcond_glue_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+Zcg*mm), stratconductive_glue_log, "conduct_glue", fullsensorLog[i], false, i, fCheckOverlaps);
      stratCu_HV_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZCu_HV*mm), stratCu_HV_log, "Cu_HV", fullsensorLog[i], false, i, fCheckOverlaps);
      stratK_HV_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZK_HV*mm), stratK_HV_log, "K_HV", fullsensorLog[i], false, i, fCheckOverlaps);
      stratAraldite2_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, translateY*mm, -wSiSensorAll/2+ZAralditel2*mm), stratAraldite2_log, "Araldite2", fullsensorLog[i], false, i, fCheckOverlaps);
      stratCfiber_phys[i] = new G4PVPlacement(0, G4ThreeVector(0.*mm, 0*mm, -wSiSensorAll/2+ZCf*mm), CfiberLog, "Cfiber", fullsensorLog[i], false, i, fCheckOverlaps);
    }
  ...
  //----- tungsten absorber
  for( int l = 0; l < 15; l++ ) absorberLog[l]->SetVisAttributes(G4Colour::Blue());
  //----- Sensor layers
  ...
  logicCalSector->SetVisAttributes(G4VisAttributes::Invisible);
  ...
  CfiberLog->SetVisAttributes(G4Color(0.847, 0.835, 0.047)); 
  for( int n = 0; n < 16; n++ ) fullsensorLog[n]->SetVisAttributes(G4VisAttributes::Invisible);  
